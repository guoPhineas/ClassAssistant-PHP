...
项目中引用了我的网站中的部分样式表和Js代码，我在里面都替换了（`staticHost`以及`host`）。基本上这些代码只是用于样式和部分控制样式的，因此你可以自行重写这部分代码。其他本项目中主要功能的JS和CSS没有替换。



> [!WARNING]
>
> 项目在开发后一直用于内部。开发时尽力保证了安全性，但仍然不能保证当中还有部分不可控的漏洞。因此不建议大范围使用。
>
